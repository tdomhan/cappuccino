...
class SMACDependency(object):
    ...
    def __str__(self):
        ...
        return " | %s in {%s}" % (self.parent_name, ", ".join(map(str, self.values)))
...
class SMACNumericalParameter(SMACParameter):
    def __init__(self, name, min_val, max_val, default=None, is_int=False, log_scale=False):
...
def subspace_to_smac(label, params, subspace, dependencies=[], escape_char_depth = "/", escape_char_choice = "@"):
    """
        Recursively convert the search space defined by dicts, lists and Parameters
        into a SMAC equivalent search space.
    ...
    if isinstance(subspace, dict):
        ...
        for idx, item in enumerate(subspace):
            ...
            subspace_to_smac(item_label, params, item, item_dependencies, escape_char_depth, escape_char_choice)
...
def convnet_space_to_smac(convnet_space):
    ...
    if isinstance(num_conv_layers, Parameter):
        ...
        num_conv_layers_param = SMACCategorialParameter("network/num_conv_layers",
                                                        values=values)
        conv_layers_fixed = False
    elif isinstance(num_conv_layers, int):
        #const value = categorical with a singe choice
        num_conv_layers_param = SMACCategorialParameter("network/num_conv_layers", values=[num_conv_layers])
        conv_layers_fixed = True
    else:
        assert False, "num_conv_layers either needs to be a Parameter or an int."
    ...
    if isinstance(num_fc_layers, Parameter):
        ...
        #const value = categorical with a singe choice
        num_fc_layers_param = SMACCategorialParameter("network/num_fc_layers", values=[num_fc_layers])
    ...
    for layer_id in range(1, convnet_space.max_conv_layers+1):
            ...
            if not conv_layers_fixed:
               for param in conv_layer_subspace:
                    num_layers_dependency = SMACDependency(num_conv_layers_param.name,
    ...
    for layer_id in range(1, convnet_space.max_fc_layers+1):
            ...
            if not fc_layers_fixed:
               for param in fc_layer_subspace:
                    num_layers_dependency = SMACDependency(num_fc_layers_param.name,
